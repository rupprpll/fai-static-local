...
;
/*
 * Play Media library video and audio items.
 */
(function ($) {

  function play_youtube(vid){
    var youtube_player = $('.media-library-playlist').find('iframe.youtube-field-player').first();

    //youtube_player[0].contentWindow.postMessage('{"event":"command","func":"pauseVideo","args":""}', '*');
    youtube_player.attr('src', "https://www.youtube.com/embed/" + vid+ "?enablejsapi=1");
    setTimeout(function(){
      youtube_player[0].contentWindow.postMessage('{"event":"command","func":"playVideo","args":""}', '*');
    }, 500);
  }
  Drupal.behaviors.media_library_video_player = {
    attach: function(context, settings) {
      $('.media-library-playlist a', context).click(function(e){
        if (e.target.tagName == 'A') {
          e.preventDefault();
          var a = $(e.target);
          var div = a.closest("div");
          let urlparams = new URL(a.attr( "href" )).searchParams;
          play_youtube(urlparams.get('v'));
          $(".media-library-playlist").find(".media-youtube.playing").removeClass("playing");
