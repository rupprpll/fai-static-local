...
(function ($) {
  ...
  // variables needed for maxlength ckeditor
  var ml = ml || {};
  ml.options = ml.options || {};

  Drupal.behaviors.fai_periodic_table = {
    attach: function (context, settings) {

      $('body').on('click', '#fai-periodic-table-add-form .delete_link', function (e) {
        var lnk_text = $(this).attr('id');
        console.log(lnk_text);
        $.ajax({
          type: "GET",
          url: '/updated-periodic-table/remove/link/' + lnk_text,
      ...
      $('body').on('click', '#fai-periodic-table-add-form a[id*="edit_link"]', function (e) {
        var link_txt = $(this).parent().prev().prev().html();
        $('#edit-field-periodic-table-link-txt').val(link_txt);

        var link_url = $(this).parent().prev().text();
        $('#edit-field-periodic-table-link').val(link_url);
      });

      $('input[type=radio][name=field_periodic_table_add_doc]').click(function(evt) {
        if(evt.target.value == 'yes') $("#info-text").show();
        else $("#info-text").hide();
      });

      if ($("[name='field_periodic_table_add_edit']").val() == 'add') {
        $('#innovation-wrapper').hide();
      }

      $("[name='field_periodic_table_add_edit']").change(function(){
        if($(this).find('option:selected').val() == 'add'){
          $('#innovation-wrapper').hide();
        }
      });

      $('#edit-field-periodic-table-description').addClass('fai-blue-bordered-wrapper');
      $('#edit-field-periodic-table-summary').addClass('fai-blue-bordered-wrapper');
      $('#edit-field-periodic-table-problem-solved').addClass('fai-blue-bordered-wrapper');
      $('#edit-field-periodic-table-benefits').addClass('fai-blue-bordered-wrapper');
      $('#edit-field-periodic-table-reason').addClass('fai-blue-bordered-wrapper');
    }
  };
