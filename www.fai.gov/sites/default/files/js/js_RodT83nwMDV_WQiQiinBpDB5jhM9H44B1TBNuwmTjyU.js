...
(function($, Drupal) {
  ...
  Drupal.behaviors.addSubHeaders = {
    attach: function(context, settings) {
      $('main', context).once('addSubHeaders').each(function () {
        let textItem = "<p class=\"toolkit-subheading\">You must be logged into MAX.gov to access the links provided below.</p>"; 
        $("#communities-of-practice-fedpm-others-title").after(textItem);
        $("#pmi-guides-standards-title").after(textItem);
        $("#acquisition-gateway-title").after(textItem);
      });
    }
  };
